...
const AppModal = ({ applicationID }) => {
  ...
  const formFields =
    role === 'admin' &&
    (applicationDetail?.admin_approval_status === 'rej' ||
      applicationDetail?.admin_approval_status === 'inc')
      ? [
                  ...
                  <Grid item xs={7}>
                    {field.type === 'file' ? (
                      ...
                      <Link
                        ...
                        onClick={() => setOpenMultiple(!openMultiple)}
                      >
                        View {field.displayName}
                        {openMultiple && (
                          <div
                            ...
                            className='multipleModal'
                          >
                            <div className='multipleModalChild'>
                              <CloseIcon
                                onClick={() => setOpenMultiple(false)}
                                className='multipleModalClose'
                              />
                              {field?.value?.length ? (
                                field?.value?.map((item, i) => {
                                  return (
                                    <Link
                                      className='multipleModalLink'
                                      href={item?.document}
                                      target='_blank'
                                      rel='noreferer'
                                    >
